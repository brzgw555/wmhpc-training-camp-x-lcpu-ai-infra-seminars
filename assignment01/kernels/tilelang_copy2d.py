...
def make_scale2d(M, N, block_M=32, block_N=32, dtype="float32"):
    ...
    def scale2d(
        ...
        with T.Kernel(grid_x, grid_y, threads=128) as (bx, by):
            ...
            #         越界部分 T.copy 会自己处理 ======
            T.copy(X[by * block_M : by * block_M + block_M, bx * block_N : bx * block_N + block_N], X_shared)

            for i, j in T.Parallel(block_M, block_N):
                X_shared[i, j] = X_shared[i, j] * 2.0

            # ====== 空 3：把算完的 tile 写回 Y 的同一位置 ======
            T.copy(X_shared, Y[by * block_M : by * block_M + block_M, bx * block_N : bx * block_N + block_N])
