...
warp & 32个thread一组的SIMT执行单元 & warp scheduler+32 lane  & 32 lane的register, shared memory  & __syncwarp() 同步;warp shuffle 指令交换寄存器数据  \\
block / CTA & SM调度的单元，一个线程组 & SM上的线程块  & shared memory  &  __syncthreads();注意原子操作\\
grid & 线程网络&  整个GPU，所有SM &  global, constant &  cudaDeviceSynchronize(),block级同步;注意原子操作\\
\bottomrule
\end{tabular}
\end{center}
...
local &  thread & thread & 片外  & 编译器  \\
shared &  block & block  & 片上 & 用户  \\
global &  所有thread host不可直接访问常规device内存,unified memory则可以& 程序  & 片外 & 用户 \\
constant & 所有thread & 程序  & 片外 & 用户 \\
L1 / L2 cache & 所有 & 硬件管理 & 片上 & 硬件管理 \\
\bottomrule
\end{tabular}
\end{center}
...
判断对错，可以顺带补一句理由。
\begin{enumerate}[label=(\alph*)]
  \item 同一个 stream 里的操作按提交顺序执行。  --正确
  \item kernel 启动后，host 代码立刻继续往下执行。 --正确
  \item unified memory 下，CPU 访问一页正被 GPU 占用的内存，会触发缺页与页迁移。  --正确 如果频繁迁移性能暴跌
\end{enumerate}
...
判断对错，可以顺带补一句理由。
\begin{enumerate}[label=(\alph*)]
  \item tile 是显存里的一块可变区域，kernel 通过指针直接改写它。  --错误。 对tile的操作基于一个新的tile而不是直接修改
  \item 对 tile 的一次运算（如两个 tile 相加）由编译器映射到 block 内的多个线程上执行。  --正确
  \item tile 模型与 SIMT 模型互斥，一个 CUDA 程序只能选一种。  --错误
\end{enumerate}
...
\midrule
并行单位 & block 里的 thread & block & block\\
编号 & \texttt{blockIdx} / \texttt{threadIdx} & ct.bid() & tl.program_id()\\
数据分工 & 线程用全局下标来划分数据 & .tiled_view()划分tile & offset偏移\\
边界处理 & \texttt{if} 判断 & 自动处理 & mask\\
\bottomrule
\end{tabular}
\end{center}
