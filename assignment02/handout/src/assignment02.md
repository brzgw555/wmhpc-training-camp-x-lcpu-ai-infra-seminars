...
(a) 一条 mma 的计算强度，分子是 $2MNK$，分母按 A、B 读入与 D 写回  (从显存读、写的开销)
的字节总和计(S016 的口径)。  正确

(b) mma.sync 是 warp 级协作指令:32 个 lane 各持 fragment 的一部分，
要求全 warp 一致地执行这条指令；有 lane 发散时行为未定义(即必须所有lane都可以到达，不能产生分支)。 正确

(c) 增大 mma 的形状 M/N/K 能提高单条指令的计算强度，而且没有代价，
所以指令形状越大越好。        错误

(d) 只要单条 mma 的计算强度低于机器平衡点，GEMM kernel 就不可能逼近
计算峰值。    错误  A/B 在 shared memory 中 可以反复读取，通过复用抬高有效强度
